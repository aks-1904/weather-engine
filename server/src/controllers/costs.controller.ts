...
import {
  calculateAdvancedVoyageCosts,
  getDetailedVoyageAnalysis,
...
export const getVoyageAnalysis = async (req: Request, res: Response) => {
  const startTime = Date.now();
  try {
    const { voyageId } = req.params;
    const { fuelPrice } = req.query;
    ...
    const analysisData = await getDetailedVoyageAnalysis(
      voyageId,
      parseFloat(fuelPrice as string)
    );
    ...
    console.error("Error generating voyage analysis:", error);
    if (error.message.includes("not found")) {
      return res.status(404).json({ success: false, message: error.message });
    }
    return res
